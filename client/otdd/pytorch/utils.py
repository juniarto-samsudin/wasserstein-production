...
def extract_dataset_targets(d):
    """ 
    从一个dataset对象中提取出targets(所有数据的标签), classes(类别indexes列表), indices(所有数据的indexes列表).
    ...
    if hasattr(dataset, 'targets'): # most torchivision datasets
        if type(dataset.targets) is torch.LongTensor:
    ...
    classes = torch.sort(torch.unique(targets)).values
